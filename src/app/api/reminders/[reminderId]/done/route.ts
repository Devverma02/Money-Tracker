...
import { markReminderDone } from "@/lib/reminders/update-reminder-state";
...
export async function POST(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const result = await markReminderDone(reminderId, user.id);
    ...
    return NextResponse.json(
      { error: "The reminder could not be marked as done." },
      { status: 500 },
    );
  }
}
